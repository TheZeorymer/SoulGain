...
const TAU: f64 = 0.020;        // Time constant (20ms)
...
impl Plasticity {
            ...
            while let Ok((current_event, current_time)) = rx.recv() {
                ...
                if let Ok(mut mem) = mem_clone.write() {
                    for (past_event, past_time) in &recent_events {
                        ...
                        if current_event == Event::Reward {
                            let reward_change = REWARD_BOOST * (-delta_t / TAU).exp();
    ...
    /// GUIDANCE LOGIC: Queries the brain for the strongest learned Opcode following 'prev'
    pub fn best_next_event(&self, prev: Event) -> Option<Event> {
        let mem = self.memory.read().ok()?;
        
        mem.weights.iter()
            .filter(|((from, to), _)| {
                // We only look for connections starting from 'prev' 
                // and pointing to an Opcode (filtering out Rewards/Errors)
                *from == prev && matches!(to, Event::Opcode { .. })
            })
